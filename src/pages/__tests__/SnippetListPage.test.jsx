...
vi.mock('../../components/snippets/SnippetCard', () => ({
  default: ({ snippet, onClick, onCopy, className }) => (
    <div 
      className={`mock-snippet-card ${className}`}
...
describe('SnippetListPage', () => {
  ...
  describe('Loading State', () => {
    it('should display loading spinner when initially loading', async () => {
      ...
      firestoreService.getSnippets.mockImplementation(() => 
        new Promise(resolve => setTimeout(() => resolve(mockSnippets), 100))
      );
  ...
  describe('Error Handling', () => {
    ...
    it('should retry loading when retry button is clicked', async () => {
      firestoreService.getSnippets
        .mockRejectedValueOnce(new Error('Network error'))
        .mockResolvedValueOnce(mockSnippets);
  ...
  describe('Copy Functionality', () => {
    it('should display copy success message when copy is successful', async () => {
      firestoreService.getSnippets.mockResolvedValue(mockSnippets);
