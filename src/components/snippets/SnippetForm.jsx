...
const SnippetForm = ({ onSubmit, loading = false, initialData = null }) => {
  ...
  const validateField = (name, value) => {
    ...
    switch (name) {
      ...
      case 'htmlContent':
        ...
        break
      case 'tags':
        // Tags validation - should be array, each tag should be reasonable length
  ...
  const handleBlur = (e) => {
    ...
    // Only show validation errors after the form has been submitted once
    if (hasSubmitted) {
    ...
    <form onSubmit={handleSubmit} className="snippet-form">
      ...
      <div className="form-group">
        ...
        <textarea
          ...
          onChange={handleInputChange}
          onBlur={handleBlur}
          className={`form-textarea ${getFieldError('htmlContent') ? 'error' : ''}`}
          placeholder="Paste your HTML code here..."
          disabled={loading}
          rows={12}
        />
        {getFieldError('htmlContent') && (
          <span className="error-message">{getFieldError('htmlContent')}</span>
        )}
      </div>
      ...
      <div className="form-group">
        ...
        <div className="tags-help">
          {formData.tags.length}/10 tags • Press Enter or click Add to add tags
        </div>
